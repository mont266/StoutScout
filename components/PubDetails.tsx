...
const PubDetails: React.FC<PubDetailsProps> = ({ pub, onClose, onRate, getAverageRating, existingUserRating }) => {
  ...
  return (
    <div className="bg-gray-800 rounded-t-2xl shadow-2xl p-4 border-t-4 border-amber-400 max-h-[80vh] flex flex-col">
      <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-700">
        <div>
          <h2 className="text-2xl font-bold text-amber-400">{pub.name}</h2>
          <p className="text-gray-400 text-sm">{pub.address}</p>
        </div>
      ...
      <div className="overflow-y-auto flex-grow">
        <div className="space-y-4 mb-6">
          <h3 className="text-lg font-semibold text-white">Average Ratings</h3>
          {pub.ratings.length > 0 ? (
            <div className="space-y-3 p-4 bg-gray-900/50 rounded-lg">
              <div className="flex justify-between items-center">
                <span className="text-gray-300">Price (Higher is Cheaper):</span>
            ...
            <p className="text-gray-400 italic text-center p-4 bg-gray-900/50 rounded-lg">No ratings yet. Be the first!</p>
          )}
        </div>
