...
const CommunityFeed = ({ onViewProfile, userLikes, onToggleLike, onLoginRequest, onViewImage, allRatings }) => {
    ...
    const fetchRatings = useCallback(async (pageNum) => {
        setLoading(true);
        setError(null);
        if (pageNum === 1) trackEvent('view_community_feed');
        else trackEvent('load_more_community_feed', { page: pageNum });
    ...
    const handleRefresh = useCallback(() => {
        if (loading) return;
        trackEvent('refresh_community_feed');
        setPage(1);
        setHasMore(true);
        fetchRatings(1);
         ...
         return (
            <div className="p-4 h-full flex items-center justify-center text-center">
                <div className="text-gray-500 dark:text-gray-400">
                    ...
                    <p className="mt-2">No ratings have been submitted yet. Be the first!</p>
                </div>
            </div>
        );
    }
