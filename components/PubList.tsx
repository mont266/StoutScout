    ...
    switch(filter) {
        case FilterType.Price:
            ...
            return <div className="flex flex-col items-end">
                ...
                <span className="text-xs text-gray-400">{avgPrice > 0 ? `${avgPrice.toFixed(1)} stars` : 'No rating'}</span>
            </div>;
        case FilterType.Quality:
            ...
            return <div className="flex flex-col items-end">
                <StarRating rating={avgQuality} color="text-amber-400" />
                 <span className="text-xs text-gray-400">{avgQuality > 0 ? `${avgQuality.toFixed(1)} stars` : 'No rating'}</span>
            </div>;
        ...
        default:
            ...
            if (distanceUnit === 'mi') {
                ...
                return <span className="text-lg font-semibold text-gray-300">{distanceInMi.toFixed(1)} mi</span>
            }
            const distanceInKm = distanceInMeters / 1000;
            return <span className="text-lg font-semibold text-gray-300">{distanceInKm.toFixed(1)} km</span>;
    }
  }

  return (
    <div className="bg-gray-800 h-full flex flex-col">
      <header onClick={onToggle} className="p-3 cursor-pointer flex justify-between items-center border-b border-gray-700 flex-shrink-0">
          <h3 className="text-lg font-bold text-white">Nearby Pubs ({pubs.length})</h3>
      ...
      {isExpanded && (
        <div className="overflow-y-auto">
          {pubs.length === 0 ? (
            ...
            <ul ref={listRef} className="divide-y divide-gray-700">
              {pubs.map((pub, index) => {
                ...
                return (
                  <li
                    ...
                    className={`p-3 cursor-pointer transition-colors flex items-center justify-between space-x-4 ${isSelected ? 'bg-amber-500/20' : 'hover:bg-gray-700/50'}`}
                  >
                    <div className="flex items-center space-x-3 min-w-0">
                        <span className={`text-lg font-bold w-6 text-center ${index < 3 ? 'text-amber-400' : 'text-gray-400'}`}>{index + 1}</span>
                        <div className="flex-1 min-w-0">
                            <p className="font-semibold text-white truncate">{pub.name}</p>
